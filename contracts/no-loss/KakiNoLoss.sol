...
contract KakiNoLoss  is OwnableUpgradeable,IKakiNoLoss{
    ...
    function initialize(IERC20 kakiToken_,IERC20 bnbToken_,IERC20 busdToken_,IERC20 kakiBNBToken_,IERC20 kakiBUSDToken_,
            IAggregatorInterface aggregator_) public initializer {
        __Ownable_init();
        ...
        _captionKAKI = 2020* _KTOKENDECIMALS;
        _captionKC=100;
    ...
    function joinFaction(uint256 factionId, uint256 id,uint256 amount) public  {
        uint256 time = getTimestamp();
        require(factionId != 0, "Cannot join faction 0.");
        require(_chapterStartTime[_chapter]>time,"please wait new _chapter!");
        require(factionId < _nextFactionId , "Cannot join uncreated factions.");
        require(_accountFactionInfo[msg.sender]._factionArr.length == 0, "Before join a faction, please leave other factions.");
        ...
        if(_accountFactionInfo[msg.sender]._bonusChapter==0)
            _accountFactionInfo[msg.sender]._bonusChapter=_chapter;
        emit JoinFaction(msg.sender, factionId, id,amount,time);
    ...
    function initAccountChapterKC(uint256 factionId) internal{
        if(_accountFactionStatus[msg.sender][factionId]._lastCheckChapter!=_chapter){
            ...
            uint256 accountKC=calFactionAllKc(stakeAmount);
            if(_factionStatus[factionId]._captain == msg.sender){
                accountKC=accountKC+calKc(_captionKC);
            }
    ...
    function updateFactioinAndAccount(uint256 factionId, uint256 id,uint256 amount) internal{
        initFactionChapterKC(factionId);
        initAccountChapterKC(factionId);

        uint256 addKc=calKc(amount*_tokenFactor[id]);
        _factionStatus[factionId]._chapterKC[_chapter]=_factionStatus[factionId]._chapterKC[_chapter]+addKc;
        _factionStatus[factionId]._stakeAmount[id]=_factionStatus[factionId]._stakeAmount[id]+amount;

        _accountFactionStatus[msg.sender][factionId]._lastCheckKC=_accountFactionStatus[msg.sender][factionId]._lastCheckKC+addKc;  
        _accountFactionStatus[msg.sender][factionId]._accountKC[_chapter]=_accountFactionStatus[msg.sender][factionId]._accountKC[_chapter]+addKc;  
        _accountFactionStatus[msg.sender][factionId]._accountKCIndex.push(_chapter);        
        _accountFactionStatus[msg.sender][factionId]._stakeAmount[id]=_accountFactionStatus[msg.sender][factionId]._stakeAmount[id]+amount; 
    
    }
    
    function addStake(uint256 factionId, uint256 id,uint256 amount) public{
    ...
    function  leaveFaction(uint256 factionId) public{
        ...
        for (uint256 id; id < _tokenAssemble.length; id++) {
            ...
            if(stakeA>0){
                _factionStatus[factionId]._stakeAmount[id]=_factionStatus[factionId]._stakeAmount[id]-stakeA;
        ...
        if(_accountFactionStatus[msg.sender][factionId]._lastCheckChapter!=_chapter){
            ...
            _factionStatus[factionId]._chapterKC[_chapter]=_factionStatus[factionId]._chapterKC[_chapter]-_accountFactionStatus[msg.sender][factionId]._lastCheckKC;
        }
        _factionStatus[factionId]._lastCheckChapter=_chapter;
    ...
    function fire(uint256 factionId,uint256 amount,  bool binary) public{
        ...
        if(binary) {
            _factionStatus[factionId]._fire[_chapter][_lastRound]._call=_factionStatus[factionId]._fire[_chapter][_lastRound]._call+amount;
            _poolState[_chapter][_lastRound]._call=_poolState[_chapter][_lastRound]._call+amount;
        } else { 
            _factionStatus[factionId]._fire[_chapter][_lastRound]._put=_factionStatus[factionId]._fire[_chapter][_lastRound]._put+amount;
    ...
    function battleDamage() public  {
        ...
        require(_roundStartTime[_chapter][_lastRound]+_roundTime <= uint256(time), "This round of trading is not over.");
        ...
        if((_chapterStartTime[_chapter])+_dayTime <= uint256(time) && _poolState[_chapter][_lastRound]._call == 0 && _poolState[_chapter][_lastRound]._put == 0) {
    ...
    function updateFactionWinnerAmount(uint256 factionId, uint256 factionChapter) internal {
        ...
        if(isToCal){
            ...
            if(_poolState[factionChapter][lastFireRound]._answer < _poolState[factionChapter][lastFireRound+1]._answer)
            ...
            if(lastFireRound>0){
                if(_poolState[factionChapter][lastFireRound-1]._answer < _poolState[factionChapter][lastFireRound]._answer)
            ...
            _factionStatus[factionId]._factionWinnerKC[factionChapter] = _factionStatus[factionId]._factionWinnerKC[factionChapter]+winner; 
        }         
    ...
    function getAccountKC(uint256 factionId,uint256 chapter) internal returns(uint256){
        ...
        for(uint256 i=len-1; i >=0; i --) {
            ...
            if(c>=chapter)
                return _accountFactionStatus[msg.sender][_nextFactionId]._accountKC[c];
        }
        return 0;
    ...
    function getBonus() public returns(uint256) {
        ...
        for (uint256 ichapter=_accountFactionInfo[msg.sender]._bonusChapter; ichapter < _chapter; ichapter++) {
            for (uint fi ; fi<_accountFactionInfo[msg.sender]._factionArr.length; fi++){
                ...
                if(_winnerKC[ichapter]>0 && faction._factionWinnerKC[ichapter]>0){
                    ...
                    bonus=bonus+_interest[ichapter] *accountKc * faction._factionWinnerKC[ichapter] / faction._totalChapterKC[ichapter]/ _winnerKC[ichapter];
